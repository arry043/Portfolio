    ...
    <div className={`flex gap-2 ${isBot ? 'justify-start' : 'justify-end'}`}>
      ...
      >
        {message.pending ? (
          ...
          <p className={`whitespace-pre-line text-sm leading-relaxed ${isBot ? 'text-zinc-300' : ''}`}>
            {message.content}
          </p>
        )}
  ...
  if (isMobile) {
    return (
      ...
      >
        ...
        <div ref={chatContainerRef} className="flex-1 space-y-3 overflow-y-auto px-3 py-3">
          {messages.map((message) => (
            <ChatMessage key={message.id} message={message} onRetry={onRetry} />
          ))}
        </div>

        <form onSubmit={onSubmit} className="border-t border-zinc-800 px-3 py-3">
  ...
  return (
    <>
      ...
      >
        ...
        <div ref={chatContainerRef} className="h-[calc(100%-8.75rem)] space-y-3 overflow-y-auto px-3 py-3">
          {messages.map((message) => (
            <ChatMessage key={message.id} message={message} onRetry={onRetry} />
          ))}
        </div>
...
const GlobalChatbot = () => {
  ...
  const [messages, setMessages] = useState([
    {
      id: 1,
      role: 'bot',
      content: "Hey, I'm Arif. Ask me about my projects, experience, skills, or certifications.",
      pending: false,
      retryable: false,
    },
  ]);
  ...
  const sendMessage = async (rawValue) => {
    ...
    if (!validation.success) {
      toast.warning(validation.error.issues[0]?.message || 'Query is invalid', 'Validation');
      return;
    }
    ...
    setMessages((previous) => [
      ...previous,
      { id: userId, role: 'user', content: query, pending: false, retryable: false },
      { id: pendingId, role: 'bot', content: '', pending: true, retryable: false },
    ]);
    setInput('');
    ...
    try {
      ...
      const answer = isFallback ? FALLBACK_MESSAGE : formatAnswer(rawAnswer);
      ...
      toast.update(toastId, {
        ...
        message: statusCode === 429 ? `Retry after ${retryAfter}s.` : getErrorMessage(error),
        actionLabel: 'Retry',
        onAction: () => {
        ...
        <AnimatePresence>
          {!isOpen ? (
            <Motion.div
              ...
              exit={{ opacity: 0, y: 6 }}
              transition={{ duration: 0.2, ease: 'easeOut' }}
              className="mb-2 max-w-40 rounded-md border border-zinc-800 bg-zinc-950/95 px-2.5 py-1.5 text-sm text-zinc-300 shadow-lg shadow-black/60"
            >
