...
const UserProfileModal = ({ open, user, onClose, onLogout, isLoggingOut = false }) => {
  const joinedDate = useMemo(() => formatJoinedDate(user?.createdAt), [user?.createdAt]);
        ...
        >
          ...
          >
            ...
            <div className="space-y-3 rounded-xl border border-zinc-800/80 bg-zinc-900/40 p-4 text-sm">
              <div>
                <p className="text-[11px] uppercase tracking-[0.12em] text-zinc-500">Name</p>
                <p className="mt-1 text-zinc-100">{toDisplayValue(user?.name)}</p>
              </div>
              <div>
                <p className="text-[11px] uppercase tracking-[0.12em] text-zinc-500">Email</p>
                <p className="mt-1 text-zinc-100">{toDisplayValue(user?.email)}</p>
              </div>
              <div>
                <p className="text-[11px] uppercase tracking-[0.12em] text-zinc-500">Joined</p>
